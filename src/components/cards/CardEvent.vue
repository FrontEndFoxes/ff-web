<template>
  <!--Card Event-->
  <div class="card-event p-4 flex flex-col justify-center items-center max-w-sm mx-auto w-full" :class="{
  'opacity-50': !event.active,
  }">
      <div class="card-event__card bg-white w-full rounded-md shadow-lg bg-cover bg-cent overflow-hidden">
          <div class="img-event w-full h-40 bg-gray-30 bg-no-repeat bg-center bg-cover"
      ...
      <a class="card-event__btn btn w-10/12 text-white -mt-6 rounded-lg overflow-hidden py-3" target="_blank"
          :href="event.link" :class="{
  ...
  'bg-gray-500 pointer-events-none cursor-not-allowed': !event.active,
          ...
          <div class="uppercase text-center text-lg">
              {{ event.active ? $t("home.btnRegister") : $t("home.btnClosed")}}
          </div>
      </a>
  </div>
</template>
