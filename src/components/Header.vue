	...
	<nav id="header" class="nav fixed w-full z-30 top-0 text-white">
		<div class="w-full mx-auto flex flex-wrap items-center justify-between">
			...
			<div class="block lg:hidden p-4">
				<button aria-label="Open Menu" @click="drawer" id="nav-toggle"
				...
				<aside id="drawer"
					...
					<ul>
						<li @click="isOpen = false">
							<router-link class="flex text-lg items-center p-4 hover:bg-blue-800 hover:text-white"
								to="/about">{{ $t("header.menu.about") }}</router-link>
						</li>
						<li @click="isOpen = false">
							<router-link class="flex text-lg items-center p-4 hover:bg-blue-800 hover:text-white"
								to="/sponsors">{{ $t("header.menu.sponsors") }}</router-link>
						</li>
						<li @click="isOpen = false">
							<router-link class="flex text-lg items-center p-4 hover:bg-blue-800 hover:text-white"
								to="/team">{{ $t("header.menu.team") }}</router-link>
						</li>
						<li @click="isOpen = false">
							<router-link class="flex text-lg items-center p-4 hover:bg-blue-800 hover:text-white"
								to="/gallery">{{ $t("header.menu.gallery") }}</router-link>
						</li>
						<li @click="isOpen = false">
							<a href="https://dev.to/FrontEndFoxes" target="_blank" rel="noopener"
								class="flex text-lg items-center p-4 hover:bg-blue-800 hover:text-white">{{ $t("header.menu.blog") }}</a>
						</li>
						<li @click="isOpen = false">
							<a href="https://etsy.com/shop/vuevixens" target="_blank" rel="noopener"
								class="flex text-lg items-center p-4 hover:bg-blue-800 hover:text-white">{{ $t("header.menu.shop") }}</a>
						</li>
						<li class="hover:bg-blue-900" v-for="(lang, i) in languages" :key="`Lang${i}`"
							@click="changeLanguage(lang.label)">
							<span class="flex text-lg items-center p-4 hover:bg-blue-800 hover:text-white">
			...
			<div class="w-full flex-grow lg:flex lg:items-center lg:w-auto hidden p-4 lg:p-0 z-20" id="nav-content">
				<ul class="list-reset lg:flex justify-end flex-1 items-center">
					<li class="hover:bg-blue-900">
						<router-link active-class="bg-blue-800" class="uppercase inline-block p-5 text-white no-underline"
							to="/about">{{ $t("header.menu.about") }}</router-link>
					</li>
					<li class="hover:bg-blue-900">
						<router-link active-class="bg-blue-800" class="uppercase inline-block p-5 text-white no-underline"
					...
					<li class="hover:bg-blue-900">
						<a class="uppercase inline-block p-5 text-white no-underline" href="https://dev.to/frontendfoxes"
							target="_blank" rel="noopener">{{ $t("header.menu.blog") }}</a>
					</li>
					<li class="hover:bg-blue-900">
						<a class="uppercase inline-block p-5 text-white no-underline" href="https://etsy.com/shop/vuevixens"
							target="_blank" rel="noopener">{{ $t("header.menu.shop") }}</a>
					</li>
					<li>
						<a class="uppercase inline-block py-2 px-4 text-white no-underline"
							href="https://github.com/frontendfoxes" aria-label="Github" target="_blank" rel="noopener">
					...
					<li class="languages hover:bg-blue-900">
						...
						<ul class="languages__dropdown">
							<li class="languages__item hover:bg-blue-900" v-for="(lang, i) in languages" :key="`Lang${i}`"
								@click="changeLanguage(lang.label)">
								<span class="flex text-lg items-center p-4 hover:bg-blue-800 hover:text-white">
									<span :class="`flag-icon ${lang.flagIcon}`"></span>
									<span class="ml-2">{{ lang.language }}</span>
